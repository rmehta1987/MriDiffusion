...
class BorutaPy(BaseEstimator, TransformerMixin):
    ...
    def _fit(self, X, y):
        ...
        imp_history_rejected = imp_history[1:, not_selected] * -10 #only keeps rejected feature importance
        self.all_imp_history=imp_history[1:,:]
        self.all_imp_history[:, not_selected] = self.all_imp_history[:, not_selected] * -10
    ...
    def check_imp_history(self):
        '''
        @added by rmehta1987
        Replace all nan values with 0 and return the mean and median importance values of this iteration
        '''
        imp_temp = np.nan_to_num(self.all_imp_history)
        return np.mean(imp_temp,axis=0),np.median(imp_temp,axis=0)
